...
abstract public class BikeCommonFlagEncoder extends AbstractFlagEncoder {
    ...
    public long acceptWay(ReaderWay way) {
        ...
        if (!highwaySpeeds.containsKey(highwayValue) && !"bridleway".equals(highwayValue)) // exclude bridleways, see http://www.openstreetmap.org/way/24064837
            return 0;
            ...
            if (surfaceSpeed != null) {
                ...
                if (highwaySpeed != null) {
                    if (surfaceSpeed > highwaySpeed) {
                        if (pushingSectionsHighways.contains(highwayTag) && !highwayTag.equals("track"))
                            speed = highwaySpeed;
                        else
                            speed = surfaceSpeed;
                    } else {
    ...
    void collect(ReaderWay way, double wayTypeSpeed, TreeMap<Double, Integer> weightToPrioMap) {
        ...
        String cycleway = getCycleway(way);
    ...
    public void applyWayTags(ReaderWay way, EdgeIteratorState edge) {
        if (isConsiderElevation()) {
            ...
            if (way.hasTag("tunnel", "yes") || way.hasTag("bridge", "yes") || way.hasTag("highway", "steps")) {
                ...
                // TODO In current version, this is mixed with the code for using an "original edge". references have been set back to getEdge()
                if (prevEdgeId != edge.getEdge()) {
